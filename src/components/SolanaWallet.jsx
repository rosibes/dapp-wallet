...
export function SolanaWallet({ }) {
  ...
  const [wallets, setWallets] = useState([]); // Stocăm adresele și cheile private
...
const deleteAllWallets = () => {
  ...
  setWallets([]); // Golește lista de portofele
...
const toggleKeyVisibility = (index) => {
  ...
  return (
    <div className="p-50 bg-white space-y-5">
      ...
      <div className=" p-10 rounded-lg shadow-lg">
        <button className="bg-black text-white rounded-lg px-6 py-2 hover:bg-gray-800"
          onClick={async function () {
            try {
              if (!mnemonic) {
                const generatedMnemonic = generateMnemonic(); // Generează un mnemonic nou
                setMnemonic(generatedMnemonic); // Salvează-l în starea mnemonic
                console.log("Generated Mnemonic:", generatedMnemonic);
              }

              console.log("Mnemonic:", mnemonic); // Log the mnemonic to ensure it's valid

              // Generate the seed from the mnemonic
              const seed = await mnemonicToSeed(mnemonic);
              console.log("Seed:", seed); // Log the derived seed

              // Derive the keypair using the first 32 bytes of the seed
              const keypair = Keypair.fromSeed(seed.slice(0, 32)); // Solana uses first 32 bytes
              console.log("Generated Keypair:", keypair);

              // Extract the 64-byte secret key
              const privateKey = keypair.secretKey;
              console.log("Private Key (Uint8Array, 64 bytes):", privateKey);

              // Convert the private key to Base58 (Solana format)
              const privateKeyBase58 = bs58.encode(privateKey);
              console.log("Private Key (Base58 format):", privateKeyBase58);

              // Increment the wallet index and add the new public key
              setCurrentIndex(currentIndex + 1);
              setWallets([...wallets, { publicKey: keypair.publicKey.toBase58(), privateKey: privateKeyBase58 }]);

              // Marcare că primul portofel a fost generat
              if (!isFirstWalletGenerated) setIsFirstWalletGenerated(true);
            } catch (error) {
              console.error("Error generating Solana wallet:", error);
              alert(`An error occurred: ${error.message}`); // Show popup for errors
            }
          }}
        >
          {isFirstWalletGenerated ? "Add Wallet" : "Generate Sol Wallet"}
        </button>
      ...
      {wallets.length > 0 && (
        <p className="text-2xl font-bold mb-4">Solana Wallets</p>
      )}
